...
- **Non-moves stay uncoded** — a code-independent guardrail keeps mere nominations ("S2?"), minimal feedback without content of its own ("Yes.", "Exactly.") and unintelligible bits ("(inaudible)") out of the coding — unless the context clearly makes a short turn a substantive move (a firm "No." as disagreement, a brief factual answer); confidence values are anchored across the full scale (90+ only for unambiguous textual evidence) instead of clustering at a default
- **Confidence on every coding** — single- and multi-coding alike: each code carries the model's **0–100 confidence**, shown inline as `CODE (NN %)`. The schema requests it in **both modes** and no longer lets a code come back without one, so even smaller models (e.g. GPT-3.5) return calibrated values instead of blanks.
...
- **Code distribution plot** — frequency of each code across the conversation, **stacked by speaker group**: one bar per code, split into the teacher's and the students' share. The total height stays the overall frequency, so "how often" remains readable and "by whom" comes on top — the interaction pattern a single-colour bar hides (e.g. invitations coming from the teacher, challenges from the students). Falls back to a single colour when only one group is coded. The DOCX/HTML report adds the exact numbers as a **code × speaker-group table** (per-code row totals and a summary row), so the split is citable, not just visible.
- **Confidence highlighting in the report** — code cells in the DOCX/HTML report are shaded at the **two edges only**: certain (≥ 90 %) and highly uncertain (< 50 %). The thresholds are the **same anchors the prompt calibrates against**, so the report never marks as certain what the model was not instructed to treat as certain. The middle band is the normal case (roughly three quarters of a real run) and stays neutral — shading it too would leave the report uniformly coloured and cost the highlighting its effect. The percentage stays in the cell and carries the detail: more precise than any band, and readable in black-and-white printing. Hand-corrected cells carry no confidence value and stay deliberately unshaded — a human decision must not look like a speculative model guess.

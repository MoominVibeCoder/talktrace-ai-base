...
- **Confidence bands in the report** — code cells in the DOCX/HTML report are shaded by how certain the model was: `●●●` certain (≥ 90 %), `●●` uncertain (50–89 %), `●` highly uncertain (< 50 %). The thresholds are the **same anchors the prompt calibrates against**, so the report never marks as certain what the model was not instructed to treat as certain. Colour *and* symbol, so the markup survives black-and-white printing, colour-vision deficiency and copy-paste. Hand-corrected cells carry no confidence value and stay deliberately unmarked — a human decision must not look like a speculative model guess.
